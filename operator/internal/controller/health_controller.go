/*
Copyright 2023.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package controller

import (
	"context"
	"errors"
	"fmt"
	"strings"
	"time"

	dfdb "dragonflydb.io/dragonfly/api/v1alpha1"
	"dragonflydb.io/dragonfly/internal/resources"
	corev1 "k8s.io/api/core/v1"
	"k8s.io/apimachinery/pkg/runtime"
	"k8s.io/apimachinery/pkg/types"
	ctrl "sigs.k8s.io/controller-runtime"
	"sigs.k8s.io/controller-runtime/pkg/client"
	"sigs.k8s.io/controller-runtime/pkg/event"
	"sigs.k8s.io/controller-runtime/pkg/log"
	"sigs.k8s.io/controller-runtime/pkg/predicate"
)

type HealthReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

//+kubebuilder:rbac:groups="",resources=pods,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups="",resources=events,verbs=get;list;watch;update;patch

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
//
// This reconcile events focuses on marking the given pods either as a `master`
// or `replica` as they go through their lifecycle. This also focus on the failing
// over to replica's part to make sure one `master` is always available.
func (r *HealthReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := log.FromContext(ctx)

	log.Info("Received", "pod", req.NamespacedName)
	var pod corev1.Pod
	err := r.Client.Get(ctx, req.NamespacedName, &pod, &client.GetOptions{})
	if err != nil && strings.HasSuffix(err.Error(), "not found") {
		// Handle Pod Deletion in a different way
		// log.Error(err, "some other error")
		return ctrl.Result{}, client.IgnoreNotFound(err)
	}

	if pod.Status.PodIP == "" {
		log.Info("Pod IP is not yet available")
		return ctrl.Result{RequeueAfter: 5 * time.Second}, nil
	}

	dbName, ok := pod.Labels["app"]
	dbNamespace := pod.Namespace
	if !ok {
		log.Error(errors.New(""), "expected app label")
		return ctrl.Result{}, err
	}

	// Retrieve the relevant
	var db dfdb.DragonflyDb
	err = r.Client.Get(ctx, types.NamespacedName{
		Name:      dbName,
		Namespace: pod.Namespace,
	}, &db)
	if err != nil {
		log.Error(err, "could not get DragonflyDb object")
		return ctrl.Result{}, err
	}

	// Given a Pod Event, What do you do?
	// If it does not have a `role`, Check with the DragonFly Object Status and do a revamp on those
	// objects.
	// It it has a `role`, Check healthyness of the pod and act if its a master
	// Pod deletion or unhealthy
	role, ok := pod.Labels[Role]
	// New pod with No role
	if !ok {
		log.Info("No role found")
		if db.Status.Phase == "" {
			// Wait for Pod Creation
			// TODO: Do better here
			duration := 1 * time.Minute
			time.Sleep(duration)
		}

		log.Info("DB Phase", "phase", db.Status.Phase)
		if db.Status.Phase == PhaseInitialized {
			// Make it ready
			log.Info("Revamp of Dragonfly object")
			if err := updateStatus(ctx, r.Client, dbName, dbNamespace, PhaseMarking); err != nil {
				log.Error(err, "could not update phase")
				return ctrl.Result{}, err
			}

			if err = findHealthyAndMarkActive(ctx, r.Client, &db); err != nil {
				log.Error(err, "couldn't find and mark active during revamp")
				return ctrl.Result{}, err
			}

			if err := updateStatus(ctx, r.Client, dbName, dbNamespace, PhaseReady); err != nil {
				log.Error(err, "could not update phase")
				return ctrl.Result{}, err
			}

		} else if db.Status.Phase == PhaseReady {
			// Probably a pod restart do the needful
			log.Info("Pod restart from a ready df instance")

			// What should this pod be?
			// If there is no active master, find and mark an healthy instance
			// if there is an active master, mark it as a replica

			// Check if there is an active master
			exists, err := activeMasterExists(ctx, r.Client, &db)
			if err != nil {
				log.Error(err, "could not check if active master exists")
				return ctrl.Result{}, err
			}

			if !exists {
				// Update status as master does not exist
				if err := updateStatus(ctx, r.Client, dbName, dbNamespace, PhaseMarking); err != nil {
					log.Error(err, "could not update phase")
					return ctrl.Result{}, err
				}

				log.Info("Master does not exist. Finding healthy and marking active")
				if err := findHealthyAndMarkActive(ctx, r.Client, &db); err != nil {
					log.Error(err, "couldn't find healthy and mark active")
					return ctrl.Result{}, err
				}

				if err := updateStatus(ctx, r.Client, dbName, dbNamespace, PhaseReady); err != nil {
					log.Error(err, "could not update phase")
					return ctrl.Result{}, err
				}

			} else {
				log.Info(fmt.Sprintf("Master exists. Marking %s as replica", pod.Status.PodIP))
				if err := markReplicaFromDb(ctx, r.Client, pod, &db); err != nil {
					log.Error(err, "could not mark replica from db")
					return ctrl.Result{}, err
				}
			}

		}

	} else {
		log.Info("Role exists already", "role", role)
		// Pod already has a role
		// Check if there was a unhealthy event.
		// Role is present, Check for healthiness
		if role == Master {
			// Master pod event. Check for health and do a failover
			if pod.Status.Phase != corev1.PodRunning {
				log.Info("Master pod is not running")
				// Pod is not running, Check if its a deletion event
				if err := findHealthyAndMarkActive(ctx, r.Client, &dfdb.DragonflyDb{}); err != nil {
					log.Error(err, "couldn't find healthy and mark active")
					return ctrl.Result{}, err
				}
			}
		} else if role == Replica {
			if pod.Status.Phase != corev1.PodRunning {
				// Mark it as a replica again
				if err := markReplicaFromDb(ctx, r.Client, pod, &db); err != nil {
					log.Error(err, "couldn't mark replica")
					return ctrl.Result{}, err
				}
			}
		}
	}

	return ctrl.Result{}, nil
}

func updateStatus(ctx context.Context, c client.Client, name, namespace, phase string) error {
	var db dfdb.DragonflyDb
	if err := c.Get(ctx, types.NamespacedName{
		Name:      name,
		Namespace: namespace,
	}, &db); err != nil {
		return err
	}

	db.Status.Phase = phase
	if err := c.Status().Update(ctx, &db); err != nil {
		return err
	}

	log.Log.Info("Updated status")

	return nil
}

// SetupWithManager sets up the controller with the Manager.
func (r *HealthReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		WithEventFilter(
			predicate.Funcs{
				UpdateFunc: func(e event.UpdateEvent) bool {
					if e.ObjectNew.GetLabels()[resources.KubernetesAppNameLabelKey] == "dragonflydb" {
						return true
					}

					return false
				},
				CreateFunc: func(e event.CreateEvent) bool {
					if e.Object.GetLabels()[resources.KubernetesAppNameLabelKey] == "dragonflydb" {
						return true
					}
					return false
				},
				DeleteFunc: func(e event.DeleteEvent) bool {
					if e.Object.GetLabels()[resources.KubernetesAppNameLabelKey] == "dragonflydb" {
						return true
					}
					return false
				},
			}).
		For(&corev1.Pod{}).
		Complete(r)
}
